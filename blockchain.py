...
def get_balance(participant):
    ...
    # Fetch a list of all sent coin amounts for the given person (empty list are returned if the person was NOT the sender)
...
def mine_block():
    ...
    copied_transactoion = open_transactions[:]
    copied_transactoion.append(rewards_transaction)
    block = {
        'previous_hash': hashed_block, 
        'index': len(blockchain),
        'transactions': copied_transactoion
    }
    ...
    if user_choice == '1':
        ...
        if len(blockchain) >= 1:
            blockchain[0] = {
                'previous_hash': '',
                'index': 0,
                'transactions': [{'sender': 'John', 'recipient': owner, 'amount': 100.0}]
            }
    elif user_choice == 'q':
